...
export default function BarcodeScannerModal({ onResult, onClose }) {
  ...
  useEffect(() => {
    ...
    ).catch((e) => setError(e?.message || 'Не удалось открыть камеру'));
    ...
    <div className="fixed inset-0 bg-black/75 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={(e) => e.target === e.currentTarget && onClose()}>
      <div className="bg-bg2 border border-border rounded-2xl w-full max-w-md overflow-hidden">
        <div className="flex justify-between items-center p-4 border-b border-border">
          <div className="font-bold text-sm">📷 Сканировать</div>
        ...
        <div className="p-4">
          ...
          <div className="text-xs text-text3 mt-3 mb-2">Если камера не распознаёт — введите вручную:</div>
          <form onSubmit={submitManual} className="flex gap-2">
            <input className="inp" autoFocus value={manual} onChange={e => setManual(e.target.value)} placeholder="Серийный номер" />

    ...
    <div>
      ...
      {stats && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-5">
          <div className="card flex items-center justify-between">
            <div><div className="text-xs text-text3 mb-1">{tt("Всего клиентов")}</div><div className="text-2xl font-black">{stats.total}</div></div>
            <Users size={22} className="text-accent2 opacity-70" />
          </div>
          <div className="card flex items-center justify-between">
            <div><div className="text-xs text-text3 mb-1">{tt("VIP клиенты")}</div><div className="text-2xl font-black text-yellow">{stats.vip_count}</div></div>
            <Star size={22} className="text-yellow opacity-70" />
          </div>
          <div className="card flex items-center justify-between">
            <div><div className="text-xs text-text3 mb-1">{tt("Общий долг")}</div><div className="text-xl font-black text-red">{Math.round(stats.total_debt_rub).toLocaleString('ru-RU')} ₽</div><div className="text-[10px] text-text3">≈ ¥{Math.round(stats.total_debt_cny).toLocaleString('ru-RU')}</div></div>
            <Wallet size={22} className="text-red opacity-70" />
          </div>
          <div className="card flex items-center justify-between">
            <div><div className="text-xs text-text3 mb-1">{tt("Оборот за 30 дней")}</div><div className="text-xl font-black text-green">{Math.round(stats.total_turnover_30d_rub).toLocaleString('ru-RU')} ₽</div></div>
      ...
      {showForm && canEdit && (
        <form onSubmit={submit} className="card mb-5 grid grid-cols-2 md:grid-cols-4 gap-3">
          ...
          <input className="inp" type="number" placeholder={tt("Скидка %")} value={form.discount_percent} onChange={e => setForm(f => ({ ...f, discount_percent: e.target.value }))} />
      ...
      <div className="grid lg:grid-cols-[240px_1fr] gap-4">
        <div className="card h-fit">
          <div className="flex items-center gap-2 font-bold text-sm mb-3"><Filter size={15} /> {tt("Фильтр")}</div>
          <div className="relative mb-3">
            <Search size={14} className="absolute left-2.5 top-1/2 -translate-y-1/2 text-text3" />
            <input className="inp pl-8 text-sm" placeholder={tt("Поиск...")} value={search} onChange={e => setSearch(e.target.value)} />
          </div>
          <div className="space-y-1 text-sm">
            {[['all', tt('Все клиенты'), clients.length], ['vip', tt('VIP клиенты'), clients.filter(c => c.category === 'vip').length],
              ['wholesale', tt('Оптовые'), wholesaleCount], ['retail', tt('Розничные'), retailCount],
              ['debtors', tt('Должники'), debtorsCount]].map(([key, label, count]) => (
          ...
          {categoryFilter !== 'all' && (
            <button onClick={() => { setCategoryFilter('all'); setSearch(''); }} className="btn btn-secondary btn-sm w-full justify-center mt-3">↺ {tt("Сбросить фильтры")}</button>
          )}
        </div>

        <div>
          <div className="flex justify-between items-center mb-3 flex-wrap gap-2">
            <div className="text-sm text-text3">{tt("Найдено")}: <b className="text-text">{filtered.length}</b> {tt("клиентов")}</div>
            <div className="flex items-center gap-2">
              <select className="inp inp-sm" value={sort} onChange={e => setSort(e.target.value)}>
                <option value="recent">{tt("Последние")}</option><option value="name">{tt("По имени")}</option>
                <option value="purchases">{tt("По покупкам")}</option><option value="debt">{tt("По долгу")}</option>
              </select>
              ...
              return (
                <Link key={c.id} to={`/clients/${c.id}`} className="card hover:border-accent/50 block">
                  ...
                  <div className="grid grid-cols-2 gap-y-1 text-xs">
                    <span className="text-text3">{tt("Баланс")}</span><span className="text-right font-mono text-green">{Math.round(c.balance_rub).toLocaleString('ru-RU')} ₽</span>
                    <span className="text-text3">{tt("Долг")}</span><span className={`text-right font-mono ${Number(c.open_debt_rub) > 0 ? 'text-red' : 'text-text3'}`}>{Math.round(c.open_debt_rub).toLocaleString('ru-RU')} ₽</span>
                    <span className="text-text3">{tt("Покупок")}</span><span className="text-right font-mono">{c.purchases_count}</span>
                    <span className="text-text3">{tt("Последняя покупка")}</span><span className="text-right text-text3">{days === null ? '—' : days === 0 ? tt('Сегодня') : `${days} ${tt('дн. назад')}`}</span>
                  </div>
                  {c.manager_name && <div className="text-[10px] text-text3 mt-2 pt-2 border-t border-border">{tt("Менеджер")}: {c.manager_name}</div>}
                </Link>
              );
            })}
            {pageItems.length === 0 && <div className="text-text3 text-sm col-span-2">{tt("Клиенты не найдены")}</div>}
          </div>

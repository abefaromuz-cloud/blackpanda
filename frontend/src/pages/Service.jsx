...
// Два разных пути ремонта: длинный (отправка в Китай) и короткий (чиним сами на месте,
// для несложных проблем). Оба начинаются с "Принят" и заканчиваются "Готов"/"Выдан".
const STAGES_CHINA = [
...
const STAGES_LOCAL = [
  ['received', '📥', 'Принят на ремонт', 'blue'],
  ['awaiting_repair', '🕒', 'Ожидает ремонта', 'orange'],
  ['repairing_local', '🛠️', 'Ремонтируется', 'red'],
  ['ready', '📍', 'Готов к выдаче', 'emerald'],
  ['done', '✅', 'Выдан клиенту', 'green'],
];
function stagesFor(repairLocation) { return repairLocation === 'local' ? STAGES_LOCAL : STAGES_CHINA; }
// Объединённый список уникальных этапов обоих путей (для статистики сверху — считаем по всем)
const ALL_STAGES_UNIQUE = [];
const seenStageIds = new Set();
[...STAGES_CHINA, ...STAGES_LOCAL].forEach(entry => {
  if (!seenStageIds.has(entry[0])) { seenStageIds.add(entry[0]); ALL_STAGES_UNIQUE.push(entry); }
});
// Общий словарь по всем этапам обоих путей — для отображения бейджа/иконки где путь неизвестен из контекста
const STAGE_MAP = Object.fromEntries([...STAGES_CHINA, ...STAGES_LOCAL].map(([id, ico, label, color], i) => [id, { ico, label, color, index: i }]));
const COLOR = {
...
const emptyItem = { kind: 'external', device_label: '', serial_input: '', found_serial: null, issue: '', is_warranty: false, cost_cny: '', technician: '', expected_date: '', repair_location: 'china' };
  ...
  return (
    <div className="bg-bg3 rounded-2xl p-4 mb-3">
      ...
      <div className="flex gap-4 mb-2 text-sm">
        <label className="flex items-center gap-1.5 cursor-pointer"><input type="radio" checked={it.repair_location !== 'local'} onChange={() => onChange({ repair_location: 'china' })} /> ✈️ {tt('Отправить в Китай')}</label>
        <label className="flex items-center gap-1.5 cursor-pointer"><input type="radio" checked={it.repair_location === 'local'} onChange={() => onChange({ repair_location: 'local' })} /> 🛠️ {tt('Починим сами')}</label>
      </div>
...
function StageTimeline({ stages, currentStage, dates = {} }) {
  const tt = useTT();
  const curIdx = stages.findIndex(([id]) => id === currentStage);
  return (
...
function ProgressBar({ stages, currentStage }) {
  ...
  const idx = stages.findIndex(([id]) => id === currentStage);
...
export default function Service() {
  ...
  const stats = useMemo(() => {
    const byStage = {};
    ALL_STAGES_UNIQUE.forEach(([id]) => { byStage[id] = 0; });
    let overdue = 0;
  ...
  async function submit(e) {
    ...
    const payload = items
      ...
      .map(it => ({
        ...
        repair_location: it.repair_location,
      }));
  ...
  function openEditDevice(orderId, item) {
    setInlineForm({
      ...
      form: {
        ...
        expected_date: item.expected_date ? item.expected_date.slice(0, 10) : '', repair_location: item.repair_location || 'china',
      },
    });
  }
    ...
    try {
      if (itemId) {
        await api.put(`/service/${orderId}/items/${itemId}`, {
          ...
          is_warranty: form.is_warranty, expected_date: form.expected_date || null, repair_location: form.repair_location,
        });
      } else {
  ...
  const STAT_CARDS = [
    ...
    ...ALL_STAGES_UNIQUE.map(([id, ico, label, color]) => [ico, tt(label), stats.byStage[id] || 0, color]),
  ];
  ...
  return (
    <div className="pb-10">
      ...
      {/* ===== Статистика сверху ===== */}
      <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-5 xl:grid-cols-10 gap-2.5 mb-6">
...
function DeviceDetailCard({ device, rate, canEdit, history, onStageClick, onEdit, onNotify, onShowHistory, onDelete, onClose }) {
  const tt = useTT();
  const stages = stagesFor(device.repair_location);
  const st = STAGE_MAP[device.stage];
  const curIdx = stages.findIndex(([id]) => id === device.stage);
  ...
  return (
    <div className="card mb-5 animate-[fadeIn_0.2s_ease] relative">
        ...
        <div className="min-w-0">
          <div className="flex justify-between items-start flex-wrap gap-2 mb-1">
            ...
            <span className="flex items-center gap-1.5 flex-shrink-0">
              <span className="badge badge-blue text-xs">{device.repair_location === 'local' ? `🛠️ ${tt('Чиним сами')}` : `✈️ ${tt('В Китае')}`}</span>
      ...
      <div className="mb-4">
        ...
        <StageTimeline stages={stages} currentStage={device.stage} dates={dates} />
      </div>

      <div className="mb-4"><ProgressBar stages={stages} currentStage={device.stage} /></div>
